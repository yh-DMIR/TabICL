...
# Dataset skipping
# -----------------------------
# Dataset folder names (or inferred dataset IDs) listed here will be skipped during discovery.
SKIP_DATASETS = {
...
def should_skip_dataset(train_path: Path) -> bool:
    """Return True if the dataset (train/test pair) should be skipped."""
...
def find_dataset_pairs(root: Path) -> List[Tuple[Path, Path]]:
    ...
    for train_path in root.rglob("*_train.csv"):
        if should_skip_dataset(train_path):
            continue
